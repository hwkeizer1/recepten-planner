  ...
  public void setEkoShoppings(List<Ingredient> ekoIngredientList, List<ShoppingItem> ekoShoppingList) {
    ...
    try {
      ...
      for (Ingredient ingredient : ekoIngredientList) {
        items.add(Arrays.asList((ingredient.getAmount() == null) ? "" : ingredient.getAmount().toString(), 
            (ingredient.getIngredientName().getMeasureUnit() == null) ? "" :ingredient.getIngredientName().getMeasureUnit().getName(), 
      ...
      .update(spreadsheetId, "A" + (startRow + 2), body)
      .setValueInputOption("USER_ENTERED")
      .execute();
      this.startRow = lastRow;
    } catch (IOException e) {
      log.error("Fout tijdens het schrijven naar Google sheets");
      e.printStackTrace();
    }
  }
  ...
  private int prepareSpreadsheet(String spreadsheetId, int listSize, int startRow, int headerIndex) throws IOException {
    ...
    Request headerCopyPasteRequest = new Request().setCopyPaste(new CopyPasteRequest()
        .setSource(new GridRange()
            .setSheetId(templateSheetId)
            .setStartRowIndex(headerIndex)
            .setEndRowIndex(headerIndex + 1)
            .setStartColumnIndex(0)
            .setEndColumnIndex(4))
        .setDestination(new GridRange()
            .setSheetId(shoppingSheetId)
