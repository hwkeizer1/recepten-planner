...
public class ShoppingListView {
	...
	private GridPane getNoStockList() {
		...
		for (ShoppingItem shoppingItem : shoppingItems.stream()
				.filter(s -> !s.getName().isStock())
				.toList()) {
			Label amountLabel = new Label(shoppingItem.getAmount() == null ? "" : shoppingItem.getAmount().toString().replaceAll("\\.0*$", ""));
			...
			Label ingredientName = new Label(
					shoppingItem.getName() == null ? "" : shoppingItem.getName().getName());
	...
	private GridPane getEkoList() {
		...
		int row = 1;
		for (ShoppingItem shoppingItem : shoppingItems.stream()
				.filter(ShoppingItem::isOnList)
				.filter(s -> s.getShopType().equals(ShopType.EKO))
				.toList()) {
			Label amountLabel = new Label(shoppingItem.getAmount() == null ? "" : shoppingItem.getAmount().toString().replaceAll("\\.0*$", ""));
			Label measureUnitLabel = new Label(
					shoppingItem.getMeasureUnit() == null ? "" : shoppingItem.getMeasureUnit().getName());
	...
	private GridPane getDekaList() {
		...
		for (ShoppingItem shoppingItem : shoppingItems.stream()
				.filter(ShoppingItem::isOnList)
				.filter(s -> s.getShopType().equals(ShopType.DEKA))
				.toList()) {
			Label amountLabel = new Label(shoppingItem.getAmount() == null ? "" : shoppingItem.getAmount().toString().replaceAll("\\.0*$", ""));
			Label measureUnitLabel = new Label(
					shoppingItem.getMeasureUnit() == null ? "" : shoppingItem.getMeasureUnit().getName());
	...
	private GridPane getMarktList() {
		...
		for (ShoppingItem shoppingItem : shoppingItems.stream()
				.filter(ShoppingItem::isOnList)
				.filter(s -> s.getShopType().equals(ShopType.MARKT))
				.toList()) {
			Label amountLabel = new Label(shoppingItem.getAmount() == null ? "" : shoppingItem.getAmount().toString().replaceAll("\\.0*$", ""));
			Label measureUnitLabel = new Label(
					shoppingItem.getMeasureUnit() == null ? "" : shoppingItem.getMeasureUnit().getName());
	...
	private GridPane getOtherList() {
		...
		for (ShoppingItem shoppingItem : shoppingItems.stream()
				.filter(ShoppingItem::isOnList)
				.filter(s -> s.getShopType().equals(ShopType.OVERIG))
				.toList()) {
			Label amountLabel = new Label(shoppingItem.getAmount() == null ? "" : shoppingItem.getAmount().toString().replaceAll("\\.0*$", ""));
			Label measureUnitLabel = new Label(
					shoppingItem.getMeasureUnit() == null ? "" : shoppingItem.getMeasureUnit().getName());
